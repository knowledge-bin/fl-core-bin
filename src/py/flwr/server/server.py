...
#         # Add research metrics collector - ADD THIS
...
# class Server:
#     """Flower server with bucketing support."""
...
#     def _reshape_weights(self, flat_weights: List[float]) -> List[np.ndarray]:
#         """
#         Unflatten weights into the shapes defined by self.model_shape,
#         without any additional scaling or clipping.
#         """
#         # 1) Compute total number of parameters expected
#         expected = sum(np.prod(shape) for shape in self.model_shape)

#         # 2) Trim or pad with zeros so length matches exactly
#         if len(flat_weights) > expected:
#             flat_weights = flat_weights[:expected]
#         elif len(flat_weights) < expected:
#             # pad with zeros if too short
#             flat_weights = flat_weights + [0.0] * (expected - len(flat_weights))

#         # 3) Reshape into layer-wise arrays
#         layered = []
#         idx = 0
#         for shape in self.model_shape:
#             size = int(np.prod(shape))
#             chunk = flat_weights[idx : idx + size]
#             arr = np.array(chunk, dtype=np.float32).reshape(shape)
#             layered.append(arr)
#             idx += size
...
#     def fit(self, num_rounds: int, timeout: Optional[float]) -> History:
#         """Run federated learning with bucketing for a number of rounds."""
#         history = History()
...
#         # Divide clients into buckets
#         num_buckets = min(self.strategy.num_groups, len(clients))
#         log(INFO, f"Dividing {len(clients)} clients into {num_buckets} buckets")
...
#         start_time = time.time()
#         # Initialize per-bucket allpub keys
#         self.bucket_allpubs = {}
        
#         for bucket_idx, bucket_clients in enumerate(buckets):
#             if not bucket_clients:  # Skip empty buckets
#                 continue
...
#             # Loop through clients in this bucket
#             for client in bucket_clients:
#                 client_vector_b = client.request_vec_b(vector_a, timeout=timeout)
#                 vector_b_list.append(self.strategy.rlwe.list_to_poly(client_vector_b, "q"))

#             # Step 2) Server sends aggregated publickey allpub to clients within the bucket
#             if vector_b_list:  # Ensure we have at least one client in the bucket
#                 allpub = vector_b_list[0]
#                 for poly in vector_b_list[1:]:
#                     allpub = allpub + poly
...
#             # Process each bucket separately and collect decrypted averages
#             bucket_plaintexts = {}
#             for bucket_idx, bucket_clients in enumerate(buckets):
#                 if not bucket_clients:
#                     continue
...
#                 # Step 3) Clients encrypt updates, server aggregates per bucket
#                 c0, c1 = bucket_clients[0].request_encrypted_parameters(request, timeout=timeout)
#                 c0sum = self.strategy.rlwe.list_to_poly(c0, "q")
#                 c1sum = self.strategy.rlwe.list_to_poly(c1, "q")
#                 for client in bucket_clients[1:]:
...
#                 # Step 4) Get decryption shares and aggregate
#                 c1sum_list = c1sum.poly_to_list()
#                 d = bucket_clients[0].request_decryption_share(c1sum_list, timeout=timeout)
#                 dsum = self.strategy.rlwe.list_to_poly(d, "t")
#                 for client in bucket_clients[1:]:
...
#                 # Step 5) Decrypt sum, wrap under modulus t, then average for bucket
#                 plaintext = c0sum + dsum
#                 plaintext_coeffs = plaintext.poly_to_list()
#                 plaintext = Rq(plaintext_coeffs, self.strategy.rlwe.t)
#                 plaintext_list = plaintext.poly_to_list()
#                 avg_weights = [round(w / len(bucket_clients)) for w in plaintext_list]
#                 bucket_plaintexts[bucket_idx] = avg_weights

#                 bucket_exec_time = time.time() - bucket_start_time
#                 print(f"Bucket {bucket_idx} Processing Time: {bucket_exec_time:.2f}s")

#             # ─── Build and broadcast the single global model ───────────────────────────
#             # Determine model size from any bucket
#             first_bucket = next(iter(bucket_plaintexts.values()))
#             model_size = len(first_bucket)

#             # Accumulate equal‐weighted average across buckets
#             global_model = [0.0] * model_size
#             num_buckets = len(bucket_plaintexts)
#             for bucket_model in bucket_plaintexts.values():
#                 for i in range(model_size):
#                     global_model[i] += bucket_model[i] / num_buckets

#             # Round to integers and log
#             global_model = [round(w) for w in global_model]
#             print(f"Global model (first 10 values): {global_model[:10]}")
#             print(f"Global model stats: min={min(global_model)}, max={max(global_model)}, mean={sum(global_model)/model_size:.2f}")
#             from flwr.common import ndarrays_to_parameters
#             layered_weights = self._reshape_weights(global_model)
#             self.parameters = ndarrays_to_parameters(layered_weights)  # <-- CRITICAL UPDATE
#             # Send the global model to all clients once
#             for client in clients:
#                 client.request_modelupdate_confirmation(global_model, timeout=timeout)
...
#             # #**************BUCKETS EVALUATION PHASE*****************

#             # # Sample evaluator clients for bucket validation
#             # available_clients = list(self._client_manager.all().values())
#             # num_evaluators = min(2, len(available_clients))
#             # evaluator_clients = random.sample(available_clients, num_evaluators)
            
#             # # Simple validation by testing each bucket model
#             # bucket_scores = {}
#             # for bucket_idx, model in bucket_plaintexts.items():
#             #     bucket_score_sum = 0.0
#             #     num_valid_scores = 0
                
#             #     for evaluator in evaluator_clients:
#             #         try:
#             #             # Use evaluate method with bucket evaluation config
#             #             # Convert model to Parameters format
#             #             from flwr.common import ndarrays_to_parameters
#             #             flat_model = model[:len(model)]  # Ensure proper length
#             #             # Reshape flat model to match original parameters shape
#             #             from flwr.common import parameters_to_ndarrays
#             #             orig_shapes = [arr.shape for arr in parameters_to_ndarrays(self.parameters)]
                        
#             #             # Build a dummy parameter structure to evaluate
#             #             params = self.parameters  # Use original structure
                        
#             #             # Evaluate bucket model
...
#             #                 }
#             #             )
#             #             eval_res = evaluator.evaluate(eval_ins, timeout=timeout)
                        
#             #             # Extract bucket score from result
#             #             if eval_res.status.code == Code.OK and "bucket_score" in eval_res.metrics:
#             #                 score = float(eval_res.metrics["bucket_score"])
#             #                 bucket_score_sum += score
#             #                 num_valid_scores += 1
#             #                 print(f"Bucket {bucket_idx} scored {score:.4f} by evaluator {evaluator.cid}")
#             #         except Exception as e:
#             #             print(f"Error evaluating bucket {bucket_idx} with client {evaluator.cid}: {str(e)}")
                
#             #     # Calculate average score for this bucket
#             #     avg_score = bucket_score_sum / max(1, num_valid_scores)
#             #     bucket_scores[bucket_idx] = avg_score

#             # # Get the model size from the first bucket
#             # first_bucket = next(iter(bucket_plaintexts.values()))
#             # model_size = len(first_bucket)
            
#             # # Create global model as weighted average of bucket models
#             # global_model = [0] * model_size
#             # total_clients = sum(len(buckets[idx]) for idx in bucket_plaintexts.keys())
            
#             # for bucket_id, bucket_model in bucket_plaintexts.items():
#             #     # Weight according to number of clients in bucket
#             #     bucket_weight = len(buckets[bucket_id]) / total_clients
#             #     for i in range(model_size):
#             #         global_model[i] += bucket_model[i] * bucket_weight
            
#             # # Round the global model values to integers
#             # global_model = [round(weight) for weight in global_model]

#             # # Print slice of the aggregated model before rounding
#             # print(f"Global model before rounding (first 10 values): {global_model[:10]}")
#             # print(f"Global model stats before rounding: min={min(global_model):.2f}, max={max(global_model):.2f}, mean={sum(global_model)/len(global_model):.2f}")

#             # # print(f"Global model after rounding (first 10 values): {global_model[:10]}")
#             # # print(f"Global model stats after rounding: min={min(global_model)}, max={max(global_model)}, mean={sum(global_model)/len(global_model):.2f}")
...
#             # # Send the global model to all clients
#             # for client in clients:
#             #     confirmed = client.request_modelupdate_confirmation(global_model, timeout=timeout)

#             # print(f"Global model created by averaging {len(bucket_plaintexts)} bucket models")
